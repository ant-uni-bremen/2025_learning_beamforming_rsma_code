...
def plot_user_number_sweep_testing_graph(
    ...
    def get_metric_key(data_dict, match_string):
        for key in data_dict[1].keys():
            if match_string in str(key):
                return key
        raise ValueError(f'Match string "{match_string}" not found in any key of data_dict[1]')
    ...
    for data_id, data_entry in enumerate(data):
        metric_key = get_metric_key(data_entry, match_string)  # match_string jetzt übergeben
...
if __name__ == '__main__':
    ...
    plot_legend = ['SAC', 'RSMA full', 'H-RSMA', ]
    plot_markerstyle = ['v','o', 's', '^']
    plot_colors = [plot_cfg.cp2['gold'],plot_cfg.cp2['blue'], plot_cfg.cp2['magenta'], plot_cfg.cp2['black']]
    plot_linestyles = ['-','-', '-', '--']
