...
#         #      'testing_rsma_genie_sweep_500_50000_new.gzip'),
#         # Path(cfg.output_metrics_path,
#         #      '01_user_distance_without_error','01_mixed_objective_error',
...
#
#         Path(cfg.output_metrics_path,
#              '01_user_distance_without_error', '01_mixed_objective_error','error_0.025',
...
#         Path(cfg.output_metrics_path,
#              '01_user_distance_without_error', '01_mixed_objective_error', 'error_0.025',
#              'testing_learned_rsma_full_usersweep_500_50000_without_error.gzip'),
#         Path(cfg.output_metrics_path,
#              '01_user_distance_without_error', '01_mixed_objective_error', 'error_0.025',
#              'testing_learned_usersweep_500_50000.gzip'),
#         Path(cfg.output_metrics_path,
...
def plot_distance_sweep_testing_graph(
    ...
    for data_id, data_entry in enumerate(data):
        ...
        alpha = alphas[data_id] if alphas is not None else 1.0
        label = legend[data_id] if legend is not None else None

        # H-RSMA ist jetzt Index 4 (durch eingefügte Referenzkurve verschoben)
        filled = (data_id == 4)
    ...
    if legend:
        # Keine explizite Label-Liste mehr übergeben – Matplotlib nimmt
        # automatisch nur die Linien, die ein label != None besitzen.
        ax.legend(ncols=2,
                loc='upper center',
...
if __name__ == '__main__':
    ...
    data_paths = [
        ...
        Path(cfg.output_metrics_path,
             '01_user_distance_without_error', '01_mixed_objective_error', 'error_0.025',
             'testing_learned_usersweep_500_50000_without_error.gzip'),
        Path(cfg.output_metrics_path,
             '01_user_distance_without_error', '01_mixed_objective_error', 'error_0.025',
             'testing_learned_rsma_full_usersweep_500_50000_without_error.gzip'),
        Path(cfg.output_metrics_path,
             '01_user_distance_without_error', '01_mixed_objective_error', 'error_0.025',
             'testing_learned_rsma_power_common_usersweep_500_50000_without_error.gzip'),
        Path(cfg.output_metrics_path,
             '01_user_distance_without_error', '01_mixed_objective_error', 'error_0.025',
             'testing_learned_usersweep_500_50000.gzip'),
        Path(cfg.output_metrics_path,
    ...
    # Reihenfolge entspricht data_paths:
    # 0: RSMA genie, 1: L-RSMA (ohne Fehler, Referenz), 2: L-SDMA, 3: L-RSMA, 4: H-RSMA
    plot_legend = [r'RSMA $\alpha*$',
                   r'L-SDMA, $\Delta {{\varepsilon}}  = 0$',
                   r'L-RSMA, $\Delta {{\varepsilon}}  = 0$',
                   r'H-RSMA, $\Delta {{\varepsilon}}  = 0$',
                   r' ',
                   r'L-RSMA, $\Delta {{\varepsilon}}  = 0.025$',
                   r'L-RSMA, $\Delta {{\varepsilon}}  = 0.025$',
                   r'H-RSMA, $\Delta {{\varepsilon}}  = 0.025$']

    plot_markerstyle = ['o','s', 'd', 'x','None', 's','d','x']

    plot_colors = [plot_cfg.cp2['black'],
                   change_lightness(plot_cfg.cp3['blue2'], 0.5),
                   change_lightness(plot_cfg.cp3['red2'], 0.5),
                   change_lightness(plot_cfg.cp3['red1'], 0.5),
                   change_lightness(plot_cfg.cp3['red2'], 0) ,
                   plot_cfg.cp3['blue2'],
