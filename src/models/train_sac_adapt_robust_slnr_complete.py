...
def train_sac_adapt_robust_slnr_complete(
    ...
    config.config_learner.algorithm_args['network_args']['num_actions'] = config.sat_nr * config.sat_ant_nr * config.user_nr
    ...
    for training_episode_id in range(config.config_learner.training_episodes):
        ...
        for training_step_id in range(config.config_learner.training_steps_per_episode):
            ...
            scaling_factors = action.reshape(robust_slnr_precoding.shape)

            # rescale SLNR precoder & normalize power
            scaled_precoding = np.multiply(scaling_factors, robust_slnr_precoding)  # hadamard product / elementwise
            normed_scaled_precoding = norm_precoder(
                precoding_matrix=scaled_precoding,
                power_constraint_watt=config.power_constraint_watt,
                per_satellite=True, sat_nr=config.sat_nr,
...
if __name__ == '__main__':
    cfg = Config()
    train_sac_adapt_robust_slnr_complete(config=cfg)
